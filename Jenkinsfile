    ...
    stages {
        ...
        stage('Preparing Infrastructure') {
            steps {
                ...
                sudo sed -i 's/^#neutron_external_interface:.*/neutron_external_interface: "dummy0"/g' /etc/kolla/globals.yml
                sudo sed -i 's/^#kolla_internal_vip_address:.*/kolla_internal_vip_address: "10.0.2.15"/g' /etc/kolla/globals.yml
                sudo sed -i 's/kolla_internal_vip_address: "10.0.2.15"/kolla_internal_vip_address: "192.168.7.15"/g' /etc/kolla/globals.yml
